...
### Step 4: Testing
- Integration test: produce → consume works
...
- 2.2: Async S3 uploads (move flush to background thread pool)
- 2.3: Metadata caching (eliminate SQLite round-trips)
- 2.4: Batch produce API (accept multiple records per gRPC call)
