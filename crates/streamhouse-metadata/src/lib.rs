...
pub trait MetadataStore: Send + Sync {
    ...
    /// List consumer groups for a specific organization.
    async fn list_consumer_groups_for_org(&self, _org_id: &str) -> Result<Vec<String>> {
        self.list_consumer_groups().await
    }

    /// Ensure a consumer group exists for a specific organization.
    async fn ensure_consumer_group_for_org(&self, _org_id: &str, group_id: &str) -> Result<()> {
        self.ensure_consumer_group(group_id).await
    }

    /// Commit an offset for a consumer group within a specific organization.
    ...
    async fn get_consumer_offsets_for_org(
        ...
        self.get_consumer_offsets(group_id).await
    }

    /// Delete a consumer group within a specific organization.
    async fn delete_consumer_group_for_org(&self, _org_id: &str, group_id: &str) -> Result<()> {
        self.delete_consumer_group(group_id).await
    }
    ...
    /// Get an organization by external identity provider ID.
    ///
    /// # Arguments
    ///
    /// * `external_id` - External identity provider organization ID
    ///
    /// # Returns
    ///
    /// - `Ok(Some(Organization))` if found
