...
impl MetadataStore for SqliteMetadataStore {
    ...
    async fn ensure_consumer_group_for_org(&self, org_id: &str, group_id: &str) -> Result<()> {
        ...
        sqlx::query(
            "INSERT OR IGNORE INTO consumer_groups (organization_id, group_id, created_at, updated_at) VALUES (?, ?, ?, ?)",
        )
        .bind(org_id)
        .bind(group_id)
        .bind(now)
    ...
    async fn get_committed_offset_for_org(
        ...
        let row = sqlx::query(
            ...
            SELECT committed_offset
            FROM consumer_offsets
            WHERE organization_id = ? AND group_id = ? AND topic = ? AND partition_id = ?
            "#,
        )
        .bind(org_id)
        .bind(group_id)
        ...
        Ok(row.map(|r| {
            use sqlx::Row;
            r.get::<i64, _>("committed_offset") as u64
        }))
    }
        ...
        Ok(rows
            ...
            .map(|r| {
                ...
                ConsumerOffset {
                    ...
                    topic: r.get("topic"),
                    partition_id: r.get::<u32, _>("partition_id"),
    ...
    async fn delete_consumer_group_for_org(&self, org_id: &str, group_id: &str) -> Result<()> {
        sqlx::query("DELETE FROM consumer_groups WHERE organization_id = ? AND group_id = ?")
            .bind(org_id)
            .bind(group_id)
            .execute(&self.pool)
            .await?;
