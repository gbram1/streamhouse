...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let partition0 = metadata
        .get_partition(TEST_ORG_ID, topic, 0)
        .await?
        .unwrap();
    let partition1 = metadata
        .get_partition(TEST_ORG_ID, topic, 1)
        .await?
        .unwrap();
    ...
    let segments0 = metadata.get_segments(TEST_ORG_ID, topic, 0).await?;
    let segments1 = metadata.get_segments(TEST_ORG_ID, topic, 1).await?;
